...
$("#findRecipientByIIN").click(function() {
    var iin = $("#iin").val(); // Получаем ИИН из поля ввода

    if (!iin) {
      alert("Пожалуйста, введите ИИН получателя.");
      return;
    }

    // Отправляем AJAX-запрос для поиска получателя по ИИН (замените на ваш фактический URL)
    $.ajax({
      url: "/recipient/" + iin,
      type: "GET",
      success: function(recipientData) {
        if (recipientData) {
          // Если получатель найден, заполняем форму получателя
          fillRecipientForm(recipientData);
        } else {
          alert("Получатель с ИИН " + iin + " не найден.");
        }
      },
      error: function(xhr, status, error) {
        console.error("Ошибка:", error);
        alert("Произошла ошибка при поиске получателя.");
      }
    });
  });

// Функция для заполнения формы получателя данными
function fillRecipientForm(recipientData) {
    // Обновляем поля формы получателя данными из recipientData
    $("#fio").val(recipientData.fio);
    $("#docId").val(recipientData.docId);
    $("#docCreationDate").val(recipientData.docCreationDate);
    $("#regionName").val(recipientData.regionName);
    $("#cityName").val(recipientData.cityName);
    $("#streetName").val(recipientData.streetName);
    $("#buildingNumberId").val(recipientData.buildingNumberId);
    $("#roomNumberId").val(recipientData.roomNumberId);
    $("#phone").val(recipientData.phone);
}
